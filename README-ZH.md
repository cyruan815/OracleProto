...
| 版本    | 头条变更                                                                                                | 默认行为                                                            |
| ------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| v3      | 宽表模式，按样本可观测性列                                                                               | 严格字母评分，无信念，无探测器                                       |
| v4      | 通过伴随 JSON 块的信念协议；概率套件（BI / NLL / MBS / ABI）                                              | `BELIEF_PROTOCOL=false`；启用前与 v3 字节一致                        |
| v5      | 离散原生转向：FSS / Cohen κ / Fleiss κ / Hamming 为主；在 $`K=5`$ 处放弃可靠性与 Murphy 三分解图            | 考试式与综合权重为头条                                              |
| v5.1    | 框架韧性：循环内强制收尾、预算丢工具、重试兜底；加宽错误关键词                                            | 强制收尾默认开启，重试兜底默认关闭                                   |
| v5.2    | 用于检索内容泄漏的二阶段 LLM 探测器                                                                     | `ENABLE_SEARCH_LEAK_FILTER=true`(默认严格、失败即丢弃)             |

迁移仅向前：每份旧 DB 在首次重新打开时通过 `ALTER TABLE ADD COLUMN` 自动迁移。由
`tests/test_db_v4_migration.py` 与 `tests/test_db_v5_migration.py` 固定。
