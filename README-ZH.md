...
- **背景与挑战：**随着大语言模型向现实世界的决策支持系统演进，其预测能力的评估面临两难困境：实时基准测试虽然纯粹但会随事件落幕而失效；而可复现的回顾性测试往往难以区分模型是真正具备预测能力，还是在预训练中已经学到了事实，且仅靠提示词让模型“假装不知道”无法建立真正的知识边界。

- **架构和方法：**我们提出了可复现的评估框架 OracleProto，它通过结合模型截断对齐的样本准入、工具级时间屏蔽、内容级泄露检测、离散答案归一化以及层次化评分等核心组件，将已确定的历史事件严谨地重构为具有时间边界的预测样本。

- **实验的效果：**在 6 种主流 LLM 上的测试表明，OracleProto 能在受控边界下有效区分模型的预测质量、稳定性与成本效益，成功将残留泄露率降低至 1% 的水平，从而将 LLM 预测从一次性评估转化为可审计、可复用的能力，并为公平的模型对比及下游 SFT（监督微调）和 RL（强化学习）提供了受控的信号源。
...
在线快速预览：[预览链接](https://online.fliphtml5.com/OracleProto/OracleProto/)
