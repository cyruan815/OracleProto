...
`forecast_eval_set.db` 直接进了仓库。它是评测的"金标准"，必须随仓库分发；任何人都能用 `git clone` 拿到完全一致的 322 道题。
...
> 拿到 `openai__gpt-5.db` 的人，不需要再去找 `forecast_eval_set.db`，也不需要找当时 metadata 是哪一版——所有评测当时的输入都在这个 DB 里。
