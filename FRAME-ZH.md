...
SHA-256 指纹把辅助泄漏探测器 $`H_{\mathrm{aux}}`$ 与运行元数据绑定，
从而使泄漏屏障本身可字节复现。每一节都回答同一个问题：实现是如何
兑现 $`\mathcal{R}`$ 的某一组成的，又由哪一个测试固定该契约？
...
| $`\hat{p}_{q,j}`$    | 信念向量（概率族配套）             | `BELIEF_PROTOCOL`（默认 `False`）               | `parser.parse_belief`（parser.py:L117）；`react.run_react` 收尾阶段              | `s{i}_belief_final`、`s{i}_belief_trace`、`s{i}_belief_parse_ok` | `test_parser_belief.py`、`test_react_reflection.py` |
...
使用三个独立信号量而不是共享单一预算，是因为三个后端各自有独立的
限速，且探测器的 QPS 预算通常只是主 LLM 预算的一部分。

写出器侧采用“每模型一个单线程异步写出任务”：`runner.run`
（runner.py:L362）为每个模型打开一个 `db.AsyncWriter`，并把该模型的
结果路由到它。因此每个单模型 DB 始终是“一个写者、多个读者”的格局，
这在 SQLite WAL 模式下是安全的。
...
工具级过滤作用在请求侧，但返回的摘要、缓存页面与聚合摘要仍可能
...
问题文本、选项与正确答案永远不会被传入。把探测器框定为“答案审计器”
会制造二阶泄漏：探测器可能据此为“伪造证据恰好与已知答案一致”这一
情形找出合理化解释，并放行该条目。
...
`failed:auth`，永不向上传播；对应条目立即被丢弃
（leak_filter.py:L281–L288）。其他可重试错误耗尽序列后，裁决变为
`failed:<kind>`，由 `LEAK_DETECTOR_FAIL_ACTION` 接管：默认 `drop`
丢弃条目，`keep` 则放行。这一默认把残余偏向“不确定即丢弃”，因为
探测器失败与条目内容无关。
...
> 符号上，例如 `[`、`\`、`]`、`^`、`_`、`` ` ``、`a`、`b`、`c`。这些
> ASCII 续接标签对 LLM 处理不友好：反引号与下划线会被 markdown 与
> 代码块吞掉，小写 `a` 与大写 `A` 在内联渲染中也容易混淆。保留该方案
> 是因为它对源数据的字母编码维持了一一对应关系，便于按字母集合评分。
> 三条强制防御措施同时启用：当选项数 >26 时，`prompts.render_user_prompt`
> 在生成 `outcomes_block` 时给标签加引号并转义；`parser.parse_answer`
...
下面的骨架保留了全部框架接线，同时短到能在一屏内读完。行内注释
标出循环必须遵守的四项契约；注释中编号引用的完整 Python 实现
位于 `react.py`。
...
async def run_react(q: Question, model: str, sample_idx: int, settings: Settings):
    ...
    # D1 兜底：若 final_raw 仍为空，用 tools=[] 重试一次扫尾。
...
CREATE TABLE run_meta (
    ...
    reflection_protocol_text  TEXT,            -- 可空
    reflection_protocol_hash  TEXT,            -- 可空
    belief_protocol_text      TEXT,            -- 可空
    belief_protocol_hash      TEXT,            -- 可空
...
| `s{i}_belief_final`               | 末步 `Belief.probabilities` 的 JSON 序列化；解析失败或 `BELIEF_PROTOCOL=False` 时为 NULL。                                                                          |
| `s{i}_belief_trace`               | 每个循环步的信念摘要 JSON 数组。                                                                                                                                    |
| `s{i}_belief_parse_ok`            | 末步信念是否合法解析（0 或 1）；独立于 `parse_ok`。                                                                                                                  |
| `s{i}_final_answer_retry_used`    | 0 或 1，当 `REACT_FINAL_ANSWER_RETRY` 兜底了空 `final_raw` 时（§5.1）置 1。                                                                                          |
...
模型的写出器。写出器每攒满 `DB_COMMIT_BATCH` 条或每秒一次刷盘
（`AsyncWriter.FLUSH_INTERVAL_S = 1.0`）；事务短小，且 SQLite 写入
通过 `await asyncio.to_thread(...)` 调用，事件循环永不阻塞。单模型
DB 因此始终是“一个写者、多个读者”的格局，在 WAL 下是安全的。
...
有六个边界值得强调。(i) AUTH 错误中止整个运行，因为继续在错误的
key 上烧预算毫无意义；`runner._run_task_with_retry` 重新抛出 `AuthError`
（runner.py:L245），外层循环取消所有任务、刷写出器并退出。
(ii) 内容策略不重试，因为重发同一道题得到的结果一致；报告会统计
每个模型累计了多少次拒绝。(iii) 拒绝不是错误：合法的 LLM 返回但未
提交 boxed 答案，属于对模型行为的一项度量，被计入统计而非 `error`。
(iv) Tavily 失败降级为 `tool_result` 错误，由 LLM 自行决定重试该查询
还是放弃，整个样本不会因此中断。(v) 探测器失败默认即丢弃，因为它
与条目内容无关。(vi) `skipped_training_cutoff` 不计入错误率，因为
它是主动的数据净化而非模型失败。
...
下表是指标栈的快速索引。给出应优先查阅的小节；§X.Y 引用指向本节
的小节。
...
| `final_answer_retry_rate`       | 框架兜底扫尾空 `final_raw` 的样本占比                                                   | `s{i}_final_answer_retry_used = 1` |
...
> 失去意义。`calibration.py` 不在分析栈中，其五个产物不会产出；概率列
> 在 `per_model_summary.md` 中保留 `†` 脚注。重新引入校准需要将 $`K`$
> 提高至至少 30。

#### 9.8.1 `belief_final IS NULL` 但 `parse_ok = 1` 时的信念兜底

无信念数据的运行与信念解析失败仍可借助退化概率向量参与正分评分：
...
| 文件                                            | 模式                                            | 内容                                    |
| ----------------------------------------------- | ----------------------------------------------- | --------------------------------------- |
| `per_model_summary.csv` 与 `.md`                | 24 基础 + 4 FSS + 4 一致性 + 7 概率 = 39 列      | 每模型一行                              |
...
| `metric_pairwise_bootstrap.csv`                 | 每指标 × 每对结果                                | 多指标两两                               |
...
`(real_model, R, C)` 或对非网格运行返回 `None`。`::` 分隔符有意避开
与厂商 slug 的冲撞，配置校验也额外拒绝 `MODELS` 中含 `::`。
...
| $`T`$、$`C`$：ReAct 循环有界、预算门控、框架优先级链、框架韧性开关                                | `test_react.py`（1432 LOC）、`test_react_reflection.py`                                                                                          |
...
| 文件                           | LOC   | 覆盖内容                                                                       |
| ------------------------------ | ----- | ------------------------------------------------------------------------------ |
| `test_react.py`                | 1432  | 完整 ReAct 循环：每个框架分支、优先级链、收尾、所有开关                         |
...
> **一句话。** 本代码库把运行单元
> $`\mathcal{R}=(\mathcal{D}, M, \kappa_M, \delta, T, C, R, \Psi, \phi, \Gamma)`$
> 与辅助探测器 $`H_{\mathrm{aux}}`$ 实现为：每个符号对应一个 Python
> 模块、每条观测对应一列 SQLite、每项指标对应一列 CSV、每条不变量
> 对应一个单元测试；报告中出现的每一个数字都可以追溯到宽表中的某
> 一行、`run_meta` 中的某个哈希、`search_calls` 中的某条审计裁决，
> 或 `tests/` 中的某个绿色测试。
