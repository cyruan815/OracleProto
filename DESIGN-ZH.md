...
尤其不擅长伪装；仅 1–5% 的标签噪声就足以击溃严格评分规则。推理侧的防御性工作得出相同
结论：单次推理的防线无法跨运行泛化，纪律必须落到一层之下，进入数据集本身。

OracleProto 的回应是把纪律推进到数据集 schema 与运行单元
$`\mathcal{R}=(\mathcal{D}, M, \kappa_M, \delta, T, C, R, \Psi, \phi, \Gamma)`$。
三条严格约束由此立即派生，本文其余每一条决策都是它们的下游。
...
把这三条约束放在心里，后续看似过严的选择就不再过严——每一项都顺着它要满足的约束自然落下。
...
右列的决策会改变*这是哪一份实现*，但不会改变*这是哪一次评测*。$`R`$、$`\Psi`$、$`\phi`$、
$`\Gamma`$ 的指纹覆盖评测；探测器的指纹覆盖辅助泄漏屏障；其余皆为工程。我们显式记录这一区
分，因为它告诉你把 OracleProto 移植到不同技术栈时，哪些旋钮可以安全替换。
...
表中标为**否**的两行是我们的主张未涵盖的残余来源。我们不假装它们不存在；任何针对
OracleProto 主张的攻击都应该针对这两行，而非上面的四行。§2.2 至 §2.6 按从“严格可执
行”到“诚实声明”的顺序镜像这些通道。
...
支撑这一选择的有两条设计原则。**能力边界对齐工具边界。** “了解世界到某一日”的能力由
系统配置决定，不应受到提示工程或模型行为的影响。让模型连“我被截断在哪一天”都看不到，
就阻止了它通过提示构造或参数注入来推断或绕过该边界。**单一、可控的失败面。** 倘若把
`end_date` 当成工具参数暴露出去，我们就必须假设模型可能忘记填写或故意填一个未来日期；
把决策保留在工具实现内部，失败模式便从“模型可能犯错”塌缩为“我们的代码可能犯错”——后者
可测、可审计、可被单元测试覆盖。
...
部分实际泄漏并把残余泄漏压到个位数。第二阶段是项目对这一两层模式的实现：算法层执行日期
过滤，语义层审计返回正文。完整规约见
...
对多选问题，项目在严格相等之外配上两条软惩罚伴随指标，但从不替换严格相等本身。

* **考试式部分得分**（`analysis/exam_score.py:L62`）遵循“任何 FP 一票否决为 0；否则
  得分为 $`|TP|/|G|`$”，即零-FP 闸门下的召回率。它让头条综合准确率在严格相等下方差
  接近零的多选桶上呈现更有层次的差异。
...
`\`、`]`、`^`、`_`、`` ` ``、`a`、`b`、`c`。这些字符对 LLM 处理不友好——反引号会被
markdown 格式化吞掉，小写 `a` 与大写 `A` 也容易混淆——但项目仍保留它们，因为这能保住
字母 ↔ 索引的一一映射。
...
当 LLM 不输出 `\boxed{...}` 或写下“我无法预测未来”时，这不是系统失败，而是对模型行为
的一项度量。三项机械后果随之产生。
...
默认值在 `config.py:L365`。原则一句话：**让能区分模型能力的桶贡献更多**。若想为简单题
加权，只需在 `.env` 中调整 `COMPOSITE_WEIGHTS_QTYPE` 或 `COMPOSITE_WEIGHTS_CTYPE`。
这是一种有立场的默认而非中立默认；上述取向对绝大多数评测场景都说得通，不同意者通过
覆盖一行 `.env` 即可解决。
...
莽”的模型置于同一成本-效益尺度上，挡住“低样本单价但高错误率”所制造的低成本假象。语
义上，$`C^{\text{per-correct}}`$ 是“一美元能买到多少难度加权的正确预测”的倒数。
...
用预算给模型的自主搜索定上限，服务两个目的。无封顶时，恶意或退化模型可无限调用并烧
穿 API 账单。封顶时返回一条错误给模型，而非抛出异常，这样 LLM 仍能基于已检索到的信息
给出“尽力答案”，并把“超预算”与“系统崩溃”这两件事分开。
...
会直接返回 400。项目在 `.env` 中维护子串列表
`LLM_REASONING_MODEL_PATTERNS=o1,o3,o4,r1,qwq`（默认值在 config.py:L241）；对匹配的
模型，调用时直接不带这两个参数。这是"把处理左移"的设计：与其在重试或错误处理路径上
识别 400，不如在请求构造阶段就解决。
...
题就退化为普通的单线程批量插入。"每 INSERT 加锁" 这条替代方案能跑，但争用时只能空烧
CPU 且没有摊销空间；编排路线则把成本搬到入队侧（便宜），而不是写入侧（昂贵）。
...
`forecast_eval_set_example.db` 直接进入仓库。它是评测的金标准示例数据集，必须随仓库
一同发布；任何人 `git clone` 都能拿到完全相同的题目。文件名（`SOURCE_DB`）与内部题
表名（`SOURCE_TABLE`，默认 `forecast_eval_set_example`）都暴露为 `.env` 参数；接入
自定义数据集时只需改这两个变量，loader 会在运行时把 `<SOURCE_TABLE>` 拼入 SQL
`FROM` 子句。表名在 Settings 阶段以 `^[A-Za-z_][A-Za-z0-9_]*$` 白名单校验
（config.py:L586），用于防止 SQL 注入；这是整个系统中唯一一处可能被注入的位置，
因此校验只需在此处进行。

"把 DB 放在独立 registry" 这条替代方案会给复现增添网络依赖；纳入 Git 之后，
`git clone` 就是唯一的安装步骤。
...
模板。任何模板改动都会重写每一份渲染后的 prompt，因此它是粗粒度的运行区分键。由
`compute_prompt_templates_hash`（db.py:L397）计算。`reflection_protocol_hash` 反映“在
ReAct 主循环中向模型注入了哪条元认知指令”，本质是搜索行为先验上的一个开关。它只在
三个轴上变化：开/关、文本是否被改、版本号。`belief_protocol_hash` 反映“模型是否在
`\boxed{...}` 之前发出结构化信念向量”，控制概率族指标是否被填入。
...
编辑后的形态调和了两个相互冲突的需求。*想知道这次运行用了哪些参数？* 已存。*想知道
key 的明文？* 永不存。要点是“可审计”与“密钥不外泄”这两件事在同一字段里共存。由
`test_db.py` 通过 `snapshot_settings` 往返并断言前缀长度与摘要长度而非原始值来固定。
...
单条日志应完整描述一个样本走过系统的路径——日志即追踪，无需 DB join。
...
`error` 字段不是“出错时随手填的自由文本”，而是固定的有限枚举：`network`、`server_5xx`、
...
SiliconFlow、本地 vLLM 都行。集成面有意保持小而标准。OpenAI 的 chat-completion 加
function-calling 协议已成事实标准，因此本项目不构建 provider 适配层，而是把适配责任
推给端点本身。这种中立性使得同一条评测流水线可以在不同 provider 托管的模型之间比较，
而无需 provider 专属的代码路径。
...
比根本没启动的评测昂贵得多。由 `test_config.py` 固定，每条规则至少有一份 fixture
同时演练 accept 与 reject 路径。
...
完整跑一遍示例数据集 × 模型数 × $`N`$ 个样本要花**几十到几百美元**。在那种规模上撞到
提示、解析器或 schema 的 bug，等于直接把钱烧掉。测试设计的核心约束：
...
这五条必须始终为绿。它们覆盖项目最容易悄悄崩坏的部分，并直接对应实现框架 $`\mathcal{R}`$
的那些组件。
...
个组件，组件失守即让整次运行单元失效。“一旦崩溃”这一列刻意写得直白；它写的是省略
测试的代价，而非运行测试的代价。完整映射（33 个测试对 11 个框架组件）见 FRAME §11.2。

### 10.3 dry-run 烟雾测试

`test_smoke_dry_run.py` 用 `httpx` stub 替换 OpenRouter 与 Tavily，端到端跑 3 题 ×
1 模型 × 1 样本。它不验证逻辑细节，只验证“管道是否还通”：检查 schema、宽表、
`messages_trace` JSON、`search_calls` 字段是否齐全。这表达了 e2e 与单元测试的分工：
单元测试验证局部正确，烟雾测试验证集成不爆。

### 10.4 测试即文档

每一条框架级不变量都有测试。当 FRAME 或 DESIGN 的散文与代码出现漂移时，测试是裁决
者。`tests/` 下的 33 个测试文件（约 13K LOC，全部离线）是“本代码库实际在做什么”最
权威的形式表达；如果你读到的与某条测试断言对不上，以测试为准。
...
delta）与 `tasks.md`（实现清单）。已命名的变更包括 `bootstrap-forecast-eval`、
`react-tavily-grid-search`、`harness-resilience-v1`、`search-leak-filter-v1`、
`add-exam-score-metric`、`composite-score-by-subtype` 与
`discrete-native-analysis-v5`。

两条原则驱动这一布局。**规约先于代码**，使错误的设计在合并前就被发现。**变更档案
与代码 diff 并存**，使日后的审计能恢复某项决定*为何*成立，而不只是看到*什么*代码
实现了它。
...
docs/CSV columns`）。纪律有三条：

* **每阶段独立通过 CI。** 不做“一个大 PR 全包”；CI 在每道闸的错位侧捕获 schema 与
  代码的不匹配。
* **每阶段可逆。** 删掉该阶段自身的代码即回到上一阶段；用一次假想回滚后重跑测试来
  验证这一点。
* **变更档案累积。** `openspec/changes/archive/` 是项目自己关于“为何走到今天”的账本；
  它比 git log 更耐久，因为每条都保留 spec / design / tasks 三份的分离，可以在
  squash-merge 之后继续存活。
...
跨团队对话中反复出现一种困惑：哪些旋钮属于“评测契约的一部分”，哪些属于“不影响科学
主张的工程调优”。这一区分很重要，因为改契约旋钮会让跨运行可比性失效，改工程旋钮则
不会。
...
契约 / 工程的二分让审查 PR 或 `.env` 变更变得机械化：§12.1 中的任一变化是*一次新评
测*，§12.2 中的任一变化是*一次 bug 修复或调优*。运行时指纹集（§7.2）使这一区分在
任何一对运行之间都是可观测的。
...
4. **原始胜于聚合。** DB 仅存观测；统计推迟到 `analysis/`。指标定义比 DB schema 演变
   得更快。
5. **默认严格，部分得分按需。** 头条指标是严格 frozenset 相等；综合用考试式部分得分；
   FSS 增加随机校正。三者在同一份原始样本上并存。
6. **可复现胜于便利。** 源数据进 Git；每个 DB 自含；六部分哈希固定指纹。
7. **可观测胜于优雅。** 完整 `messages_trace` 默认开；进度日志一行一样本；逐调用审计
   字段持久化探测器裁决。
8. **失败要分类。** 错误用有限枚举；每一类在报告中都有自己的格子。
9. **配置即契约。** `.env` 一份决定一切；CLI 标志极简；`config_snapshot` 持久化前先做
   脱敏；契约 / 工程二分（§12）告诉你哪种 `.env` 变化会让比较失效。
10. **测试守护昂贵之物。** 五条 CI 红线一对一映射到运行单元 $`\mathcal{R}`$ 的组件；
    dry-run 烟雾测试验证集成；昂贵的失败被前移到本地。
11. **框架就是契约。** 每个工程决策都按“加强还是削弱 $`\mathcal{R}`$”接受裁判；只有
    清晰划界的逃生口（`LEAK_DETECTOR_FAIL_ACTION=keep`、`WRITE_MESSAGES_TRACE=false`、
    A/B 开关）才允许便利压过框架，它们本身也留下审计轨迹。
12. **早失败，先于任何可计费调用。** 启动校验在任何 LLM 或 Tavily 接触之前拒绝错配；
    未启动的评测比悄悄被错算的评测更便宜。
13. **靠编排消除竞态，不靠锁。** 每 DB 一个 writer，每 writer 一个 queue，批量 commit。
14. **阶段化变更。** 每次 openspec 变更以可逆阶段发布；变更档案是项目关于“为何走到
    今天”的账本。
15. **三个独立指纹，而非一个。** 模板、反思协议、信念协议各自独立哈希，使消融研究
    可以沿单一轴配对运行。
...
本代码库中考虑过并被否决的所有替代方案的合并列表，按决策领域排序。每一行引用详细
论证所在的小节。
...
如果你是项目新人，建议按以下顺序阅读。这一顺序经过设计：每一步都为下一步提供语言。
第 1–3 步建立概念模型；第 4–7 步把它落到代码里；第 8–9 步交给你测试契约与规约档案。
...
> **一句话总结：** OracleProto 用工程纪律守护科学严谨。每一道看似过分的约束之所以
> 存在，是因为放弃它就会得到一份在最终报告中没有真实含义的数字。信息边界属于数据
> 的一部分，而非提示；运行单元 $`\mathcal{R}`$ 是契约，而非配置；审计轨迹是报告的
> 根基，而非它的附录。
