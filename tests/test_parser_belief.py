...
# ---- 26+ option corner case -------------------------------------------------


def test_26plus_options_letter_set() -> None:
    """The DB ships at least one question with > 26 options; the letter set
    must include the extended ASCII labels (`[`, `\\`, `]`, `^`, `_`,
    `` ` ``, lowercase `a`...) per `chr(ord('A') + i)`."""
    ...
    conn.close()
    assert rows, "expected at least one >26 option fixture in the bundled DB"
    ...
    for r in rows:
        ...
        # Build a uniform prior over all letters — must succeed under multi-class.
        letters = [chr(ord("A") + i) for i in range(n_opts)]
        probs = {ltr: 1.0 / n_opts for ltr in letters}
        out = parse_belief(_wrap(_payload(probs)), q)
        assert out is not None, f"failed on q={q.id} with {n_opts} options"
        assert set(out.probabilities.keys()) == set(letters)

        # Removing any single letter MUST be rejected (letter set mismatch).
        truncated = {k: v for k, v in probs.items() if k != letters[-1]}
        assert parse_belief(_wrap(_payload(truncated)), q) is None
